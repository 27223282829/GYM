...
<main>
    ...
    <div class="container py-4">
     ...
     <div class="container py-4">
       <h2>Listado de Roles </h2>
       <a href="{{url('rol/create')}}" class="btn btn-primary btn-sm">Nuevo registro</a>
       <table class="table table-light">
        <thead class="thead-light">
            <tr>
                <th>#</th>
        ...
        <tbody>
            ...
            <tr>
                <td>{{ $rols->id}}</td>
                <td>{{ $rols->Rol}}</td>
                <td><a href="{{url('rol/'.$rols->id.'/edit')}}" class="btn btn-warning btn-sn">Editar</a></td>
                <td><form action="{{ url('rol/'.$rols->id)}}" method="post">
                    {{ method_field("DELETE") }}
                    @csrf
